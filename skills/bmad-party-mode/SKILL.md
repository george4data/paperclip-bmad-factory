...
- `--parallel` — Run all subagents concurrently. This is faster but consumes more LLM quota/rate-limits at once. Use only when speed is critical and your provider has high limits.
- `--sequential` — (Default) Run subagents one after another. This protects your rate limits and allows agents to react to what others have said in the same round (cross-talk).
...
1. **Parse arguments** — check for `--model`, `--parallel`, and `--solo` flags. By default, assume **sequential mode** unless `--parallel` is specified.
...
{if running in sequential mode, include the responses from agents who spoke before this one in the current round — otherwise omit this section}
...
**Sequential Mode (Default)** — Spawn agents one at a time. After each agent responds, you MUST present their response to the user immediately, then proceed to spawn the next agent, passing the previous agents' responses into their context for "cross-talk."

**Parallel Mode (`--parallel`)** — Spawn all agents in parallel — put all Agent tool calls in a single response so they run concurrently. Note that in this mode, agents cannot hear what others in the same round are saying.

**Solo mode (`--solo`)** — Skip spawning. Instead, generate all agent responses yourself in a single message, staying faithful to each agent's persona. Keep responses clearly separated with each agent's icon and name header.
